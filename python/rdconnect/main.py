...
def main(sqlContext, sc, configuration, chrom, nchroms, step, somaticFlag):
    ...
    if("annotateFullDenseMatrix" in step):
        ...
        for ii in nmatrix:
            ...
            variants = annotations.annotateInternalFreq(hl, variants, configuration["intFreq"] + "/variants" + str(chrom) + ".ht", destination + "/annotateInternalFreq/" + fileName, return_matrix = False)
            current_dir = destination + "/annotateInternalFreq/" + fileName
            
            print(" VEP")
            variants = hl.methods.read_matrix_table(current_dir)
            variants = variants.key_rows_by(variants.locus, variants.alleles)
            variants = annotations.annotateVEP(hl, variants, utils.buildDestinationVEP(destination, fileName, somaticFlag), configuration["vep"], number_partitions, return_matrix = False)
            current_dir = utils.buildDestinationVEP(destination, fileName, somaticFlag)
            
            print(" dbNSFP")
            variants = hl.read_matrix_table(current_dir)
            variants = annotations.annotateDbNSFP(hl, variants, utils.buildFileName(configuration["dnNSFP_path"], chrom), utils.buildDestinationNSFP(destination, fileName, somaticFlag), return_matrix = False)
            current_dir = utils.buildDestinationNSFP(destination, fileName, somaticFlag)
                
            print(" dbCADD")
            variants = hl.read_matrix_table(current_dir)
            variants = annotations.annotateCADD(hl, variants, utils.buildFileName(configuration["cadd_path"], chrom), utils.buildDestinationCADD(destination, fileName, somaticFlag), return_matrix = False)
            current_dir = utils.buildDestinationCADD(destination, fileName, somaticFlag)

            print(" ClinVar")
            variants = hl.read_matrix_table(current_dir)
            variants = annotations.annotateClinvar(hl, variants, utils.buildFileName(configuration["clinvar_path"],""), utils.buildDestinationClinvar(destination, fileName, somaticFlag), return_matrix = False)
