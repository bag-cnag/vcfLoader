...
def transform(dataset,destination,chrom):
    dataset.annotate_variants_expr([
        ...
        'va.samples = gs.filter(x=> x.dp >7 && x.gq> 19).map(g => {gq: g.gq, dp : g.dp, gt:intToGenotype(g.gt), gtInt : g.gt, adBug : g.ad, ad: truncateAt(g.ad[1]/g.ad.sum.toFloat,2), sample : s}).collect()',
        'va.pos = v.start',
        'va.ref= v.ref',
