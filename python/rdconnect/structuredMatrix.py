...
def append_to_sparse_matrix(self = None, config = None, hl = None, log = VoidLog(), experiments = []):
	...
	experiments_selection = self.config['process/experiments_selection']
	...
	if experiments_selection=="manual":
	    ...
	    clean_to_process = [ { 'file': experiments_folder+'/{}.{}.g.vcf.bgz'.format(x.strip(), chrom_str), 'id': x.strip() } for x in experiments ]
